...
internal class GenerateCurrentMongoSecretsCommand : ICommand<GenerateCurrentMongoSecretsRequest>
{
    private readonly UserSecretsService _userSecretsService;

    public GenerateCurrentMongoSecretsCommand()
    {
    ...
    public async Task ExecuteAsync(GenerateCurrentMongoSecretsRequest request)
    {
        ...
        foreach (var project in projectInfos)
        {
            ...
            string secretFilePath = ProjectPaths.GetUserSecretsPath(project.SecretId);
                
            // Only proceed if the secret file exists
            if (File.Exists(secretFilePath))
            {
                string secretContent = await File.ReadAllTextAsync(secretFilePath);
            ...
            {
                Console.WriteLine($"  Warning: Secret file not found for project: {project.Name}");
            }
        }
    }
}